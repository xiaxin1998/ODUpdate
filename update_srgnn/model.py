...
class SessionGraph(Module):
    def __init__(self, opt, n_node):
        ...
        self.gnn = GNN(self.hidden_size, opt.step)
    ...
    def forward_test(self, alias_inputs, A, items, mask):
        hidden = self.embedding(items)
        hidden = self.gnn(A, hidden)
        get = lambda i: hidden[i][alias_inputs[i]]
        seq_hidden = torch.stack([get(i) for i in torch.arange(len(alias_inputs)).long()])
        logits = self.compute_scores(seq_hidden, mask)
        return logits
...
def forward(model, i, data):
    ...
    get = lambda i: hidden[i][alias_inputs[i]]
    seq_hidden = torch.stack([get(i) for i in torch.arange(len(alias_inputs)).long()])
    logits = model.compute_scores(seq_hidden, mask)
    loss_func = nn.CrossEntropyLoss()
    loss = loss_func(logits, targets-1)
    return targets, loss
...
def train_test(model, train_data, test_data, epoch, opt):
    ...
    for i in slices:
        ...
        A = trans_to_cuda(torch.Tensor(A).float())
        mask = trans_to_cuda(torch.Tensor(mask).long())
        scores = model.forward_test(alias_inputs, A, items, mask)
